...
const page = () => {
    ...
    useEffect(() => {
        firebase.fetchNVQData((data) => {
            const filteredData = removeDuplicates(data);
            setNvqData(filteredData);
        }, setIsLoading);
    }, []);
    ...
    const handleViewDetails = (id) => {
        setViewDetailsId(id);
        router.push(`/admin/nvq-details/${id}`);
    };

    let auto = 0;
    let autoFilteredData =
        nvqData &&
        nvqData.filter((user) => {
            return user.submitType === "auto";
        });

    let total = nvqData.length;
    auto = autoFilteredData.length;
    ...
    const handleSelectAll = () => {
        if (selectedUsers.length === nvqData.length) {
            setSelectedUsers([]);
        } else {
            setSelectedUsers(nvqData.map((user) => user.id));
        }
    };

    const handleDeleteSelected = async () => {
        if (selectedUsers.length === 0) {
            toast.error("No NVQ messages selected for deletion!");
            return;
        }
        ...
        try {
            ...
            await firebase.deleteNvqMessages(selectedUsers, setIsDeleting);
                        ...
                        <tr className="bg-gray-100">
                            <th className="border border-gray-300 px-4 py-2">
                                <input
                                    ...
                                    checked={selectedUsers.length === nvqData.length}
                                />
                            </th>
                            ...
                            nvqData.map((user, index) => (
                                <tr key={user.id} className="hover:bg-gray-100">
                                    ...
                                    <td className="border border-gray-300 px-4 py-2 flex gap-2 items-center">
                                        {index + 1}
                                        <p
                                            className={`before:content-['•'] before:${user.submitType === "auto"
                                                ? "text-orange-500"
                                                : "text-green-500"
                                                } before:text-2xl before:mr-2`}
                                        ></p>
                                    </td>
